...
class VisitorIdFilterTest {
    ...
    @Mock
    private SentinelSecurityService sentinelSecurityService;

    @Mock
    private GatewayFilterChain chain;

    @InjectMocks
    private VisitorIdFilter visitorIdFilter;
    ...
    void testFilter_ExistingValidCookie() {
        ...
        StepVerifier.create(visitorIdFilter.filter(exchange, chain))
                .verifyComplete();

        // Verify that the chain was called with a mutated exchange that has the header
        verify(chain).filter(
                argThat(ex -> extractedUuid.equals(ex.getRequest().getHeaders().getFirst(Constants.VISITOR_ID))));
    }
    ...
    void testFilter_NoCookie_GeneratesNewOne() {
        ...
        ResponseCookie responseCookie = exchange.getResponse().getCookies().getFirst(Constants.VISITOR_ID);
