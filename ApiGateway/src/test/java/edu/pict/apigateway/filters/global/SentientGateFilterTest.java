...
class SentientGateFilterTest {
    ...
    @InjectMocks private SentientGateFilter sentientGateFilter;
    ...
    void testFilter_SuccessResponse_SendsLogEvent() {
        ...
        StepVerifier.create(sentientGateFilter.filter(exchange, chain)).verifyComplete();

        verify(kafkaTemplate)
                .send(eq(KafkaTopics.USER_LOGS.topic()), eq(uuid), any(LogEvent.class));
        verify(kafkaTemplate, never())
                .send(eq(KafkaTopics.SECURITY_EVENTS.topic()), anyString(), any());
    }
    ...
    void testFilter_ErrorResponse_SendsLogAndSecurityEvents() {
        ...
        verify(kafkaTemplate)
                .send(
                        eq(KafkaTopics.SECURITY_EVENTS.topic()),
                        eq(uuid),
                        any(SecurityAlertEvent.class));
    }
