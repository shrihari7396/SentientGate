...
# Apply the ConfigMap configurations to initialize dummy service settings in the cluster
kubectl apply -f "${SCRIPT_DIR}/config.yml"
# Apply the Deployment and Service configurations to run and expose the Dummy Service container
kubectl apply -f "${SCRIPT_DIR}/deployment.yml"
# Apply the HorizontalPodAutoscaler to enable dynamic autoscaling for the Dummy Service
