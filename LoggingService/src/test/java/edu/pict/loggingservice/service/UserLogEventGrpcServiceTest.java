...
class UserLogEventGrpcServiceTest {
    ...
    @Mock private LogFetchStrategyResolver strategyResolver;

    @Mock private StreamObserver<UserLogEventResponse> responseObserver;

    @InjectMocks private UserLogEventGrpcService grpcService;
    ...
    void testGetUserEvents() {
        ...
        UserLogEventsRequest request =
                UserLogEventsRequest.newBuilder()
                        .setUuid(uuid)
                        .setDuration(durationMinutes)
                        .build();

        GatewayLogEntity logEntity =
        ...
        when(strategyResolver.fetchLogs(eq(uuid), any(Instant.class)))
                .thenReturn(List.of(logEntity));

        grpcService.getUserEvents(request, responseObserver);

        ArgumentCaptor<UserLogEventResponse> responseCaptor =
                ArgumentCaptor.forClass(UserLogEventResponse.class);
    ...
    void testGetUserEvents_WithNullFields_MapsToEmptyStrings() {
        String uuid = "test-uuid";
        UserLogEventsRequest request =
                UserLogEventsRequest.newBuilder().setUuid(uuid).setDuration(60).build();
