...
class AnomalyDetectionServiceTest {
    ...
    void testAnalyze_AnomalyDetected() {
        ...
        request.setHistory(List.of(
                LogEvent.builder().statusCode(500).timestamp(1000L).path("/admin").clientIp("1.1.1.1").build(),
                LogEvent.builder().statusCode(403).timestamp(2000L).path("/admin/config").clientIp("2.2.2.2").build()
        ));
        ...
        StepVerifier.create(anomalyDetectionService.analyze(request))
                .expectNextMatches(response -> 
                    ...
                    "AI_BEHAVIORAL_ANOMALY".equals(response.getPatternDetected()) &&
    ...
    void testAnalyze_HighSensitivityRoutes() {
        when(ollamaService.predictAnomalyScore(anyString())).thenAnswer(invocation -> {
        ...
        AnomalyDetectionRequest request = new AnomalyDetectionRequest();
        request.setHistory(List.of(
                LogEvent.builder().path("/admin/users").build()
        ));

        StepVerifier.create(anomalyDetectionService.analyze(request))
                .expectNextMatches(response -> response.isAnomaly() && response.getConfidenceScore() == 0.9)
                .verifyComplete();
    }
    ...
    void testAnalyze_MediumSensitivityRoutes() {
        ...
        AnomalyDetectionRequest request = new AnomalyDetectionRequest();
        request.setHistory(List.of(
                LogEvent.builder().path("/auth/login").build()
        ));

        StepVerifier.create(anomalyDetectionService.analyze(request))
                .expectNextMatches(response -> !response.isAnomaly() && response.getConfidenceScore() == 0.5)
                .verifyComplete();
    }
    ...
    void testAnalyze_LowSensitivityRoutes() {
        ...
        AnomalyDetectionRequest request = new AnomalyDetectionRequest();
        request.setHistory(List.of(
                LogEvent.builder().path("/public/info").build()
        ));

        StepVerifier.create(anomalyDetectionService.analyze(request))
                .expectNextMatches(response -> !response.isAnomaly() && response.getConfidenceScore() == 0.1)
                .verifyComplete();
    }
}
