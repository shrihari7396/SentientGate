...
# per-service invocation lives in exactly one place — this replaces the 14
# near-identical per-service run.sh/test.sh leaf scripts that used to duplicate
# the same `./mvnw` (or `npm`) commands across every module.
#
