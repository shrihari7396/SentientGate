...
const PipelineStep = ({ icon: Icon, title, description, status = 'active', delay = 0 }) => (
    ...
    >
        <div className={`glass-card p-10 rounded-[2.5rem] border-white/5 relative z-10 h-full flex flex-col ${status === 'active' ? 'border-purple-500/10 shadow-[0_0_50px_rgba(168,85,247,0.05)]' : 'opacity-40 grayscale'}`}>
            ...
            <div className="flex items-center gap-5 mb-8">
                <div className={`p-4 rounded-2xl ${status === 'active' ? 'bg-purple-600/10 text-purple-400 group-hover:purple-glow' : 'bg-slate-500/10 text-slate-500'} border border-white/5 transition-all duration-500 group-hover:scale-110`}>
                    <Icon className="w-8 h-8" />
                </div>
                <div>
                    <h4 className="text-lg font-black tracking-tight text-white">{title}</h4>
                    <span className="text-[10px] uppercase tracking-[0.3em] text-slate-600 font-black">SENTINEL_NODE</span>
                </div>
            </div>
            ...
            <div className="mt-8 flex items-center justify-between">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-600 border border-white/5 px-3 py-1 rounded-full">Active Mode</span>
    ...
    return (
        <div className="space-y-16 animate-in pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                <div className="px-6 py-2.5 glass rounded-2xl border-purple-500/20 shadow-2xl">
            ...
            <div className="flex flex-col lg:flex-row gap-12 py-10 relative">
                <div className="absolute top-1/2 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-500/10 to-transparent lg:block hidden -translate-y-1/2 z-0"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-12">
                <div className="lg:col-span-7 glass-card p-10 rounded-[3rem] border-white/5 bg-gradient-to-br from-blue-500/5 to-transparent relative overflow-hidden group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                        {[
                            ...
                            <div key={node.name} className="p-6 rounded-[2rem] bg-white/[0.03] border border-white/5 flex flex-col gap-4 hover:bg-white/5 transition-colors group/node">
                                <div className="flex justify-between items-center">
                                    <div className={`p-2.5 rounded-xl bg-white/5 ${node.color} group-hover/node:purple-glow transition-all`}>
                                        <node.icon className="w-5 h-5" />
                                    </div>
                                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{node.status}</span>
                                </div>
                                <p className="font-bold text-slate-200 tracking-tight text-lg">{node.name}</p>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="lg:col-span-5 glass-card p-10 rounded-[3rem] border-white/5 relative overflow-hidden flex flex-col justify-center gap-8">
                    ...
                    <p className="text-base text-slate-500 leading-relaxed font-medium relative z-10">
                    ...
                    <div className="p-6 rounded-2xl bg-purple-500/5 border border-purple-500/10 italic text-[11px] text-slate-500 leading-relaxed relative z-10">
