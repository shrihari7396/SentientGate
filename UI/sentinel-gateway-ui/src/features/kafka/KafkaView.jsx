...
import { Zap, Activity, ShieldCheck, AlertTriangle, Box, Fingerprint, RefreshCcw } from 'lucide-react';
...
const KafkaView = () => {
    ...
    const { data: healthData, isLoading: healthLoading, isError: healthError, isFetching } = useQuery({
        queryKey: ['kafkaHealth', activeUrl],
        queryFn: () => actuatorApi.getHealth(activeUrl).then(res => res.data),
        enabled: !!activeUrl,
        refetchInterval: 5000,
        onError: () => toast.error('Failed to resolve Kafka Actuator endpoint. Check connection.')
    });
    ...
    // Extracting nested kafka health details if available from Spring Boot actuator
    const kafkaDetails = healthData?.components?.kafka || healthData?.components?.binders?.details?.kafka;
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-10 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
                <div className="relative group">
                    <div className="absolute -inset-4 bg-gradient-to-r from-purple-500 to-fuchsia-500 rounded-3xl blur-2xl opacity-10 group-hover:opacity-20 transition-opacity duration-1000" />
                    <div className="relative">
                        <h2 className="text-4xl lg:text-5xl font-black tracking-tighter leading-none flex items-center gap-4">
                            KAFKA <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-fuchsia-400">CLUSTER</span>
                        </h2>
                        <p className="text-slate-500 mt-4 font-medium text-lg max-w-xl leading-relaxed">
                            Monitor event streaming nodes, topic distribution, and consumer group health.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="glass-card p-6 md:p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl relative overflow-hidden">
                ...
                <form onSubmit={handleConnect} className="relative z-10 flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative group">
                        <Fingerprint className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-purple-500 transition-colors duration-300" />
                        <input
                            type="text"
                            value={actuatorUrl}
                            onChange={(e) => setActuatorUrl(e.target.value)}
                            placeholder="Data Node Actuator URL (e.g. http://localhost:8082/actuator)"
                            className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl pl-16 pr-6 py-4 text-sm font-mono text-slate-800 dark:text-slate-200 focus:outline-none focus:border-purple-500/50 focus:bg-white dark:focus:bg-white/10 transition-all placeholder:text-slate-400"
                            required
                        />
                    </div>
                    <button
                        ...
                        className="px-10 py-4 bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-500 hover:to-fuchsia-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-purple-500/25 transition-all flex items-center justify-center gap-3 shrink-0"
                    >
            ...
            <AnimatePresence mode="wait">
                {activeUrl && healthError && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="glass-card p-10 rounded-[3rem] text-center space-y-4">
                        ...
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Broker Unreachable</h3>
                        <p className="text-slate-500">The Kafka actuator endpoint refused connection or timed out.</p>
                    </motion.div>
                )}

                {activeUrl && !healthError && healthData && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between group overflow-hidden relative">
                                ...
                                <div className="z-10 mt-auto">
                                    <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-1">Cluster State</h3>
                            ...
                            <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between group overflow-hidden relative md:col-span-2 lg:col-span-2">
                                ...
                                <div className="grid grid-cols-2 gap-8 z-10">
                                    <div>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Kafka Client ID</p>
                                        <p className="font-mono text-sm text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-white/5 p-3 rounded-xl border border-slate-200 dark:border-white/5 truncate">
                                            {kafkaDetails?.details?.clientId || 'producer-sentinel-1'}
                                        </p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Cluster ID</p>
                                        <p className="font-mono text-sm text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-white/5 p-3 rounded-xl border border-slate-200 dark:border-white/5 truncate">
                                            {kafkaDetails?.details?.clusterId || 'A4_B9_C2_D0_E8'}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-8 flex items-center justify-between border-t border-slate-200 dark:border-white/5 pt-6 z-10">
                                    ...
                                    {isFetching && <RefreshCcw className="w-4 h-4 text-purple-500 animate-spin" />}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {[
                                { label: 'Topics Active', value: '14', unit: 'Streams', trend: '+2', color: 'from-blue-500 to-indigo-500' },
                                { label: 'Global Partitions', value: '1,248', unit: 'Chunks', trend: 'Stable', color: 'from-purple-500 to-fuchsia-500' },
                                { label: 'Message Latency', value: '4.2', unit: 'ms/avg', trend: '-0.3ms', color: 'from-emerald-500 to-teal-500' },
                                { label: 'Consumer Sync', value: '99.9', unit: '% Linked', trend: 'Optimal', color: 'from-orange-500 to-rose-500' }
                            ].map((stat, idx) => (
                                <motion.div
                            ...
                            <div className="flex w-full max-w-2xl justify-between items-center z-10 px-8">
                                <div className="flex flex-col items-center gap-4">
                                    <div className="w-20 h-20 rounded-[1.5rem] bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 flex flex-col items-center justify-center text-indigo-500 shadow-lg">
                                        <span className="text-[10px] font-black uppercase tracking-widest mb-1">Bytes In</span>
                                        <span className="text-xl font-bold tracking-tighter">1.2G</span>
                                    </div>
                                </div>

                                <div className="flex-1 px-8 relative h-10 flex items-center justify-center">
                                    <div className="w-full h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 rounded-full overflow-hidden relative">
                                        <div className="absolute inset-0 bg-white/40 w-1/4 animate-[shimmer_2s_infinite]" />
                                    </div>
                                    <div className="absolute -top-4 bg-purple-100 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20 px-3 py-1 rounded-full text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest">
                                        34,228 msg/sec
                                    </div>
                                </div>

                                <div className="flex flex-col items-center gap-4">
                                    <div className="w-20 h-20 rounded-[1.5rem] bg-fuchsia-50 dark:bg-fuchsia-500/10 border border-fuchsia-200 dark:border-fuchsia-500/20 flex flex-col items-center justify-center text-fuchsia-500 shadow-lg">
                                        <span className="text-[10px] font-black uppercase tracking-widest mb-1">Bytes Out</span>
                                        <span className="text-xl font-bold tracking-tighter">4.8G</span>
                                    </div>
                                </div>
                            </div>
                        </div>
